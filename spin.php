<?php 
    ...
    if(isset($_POST['btn_login'])){
        ...
        $rs=DP::run_query("UPDATE `toshiba_lucky_spin`.`logs` SET `channels`=?,`spin_quantity` = ?, `ticket_stamp` = ?, `ticket_seri` = ? WHERE (`id_log` = ?);",[(isset($_POST['channel'])?implode(",",$_POST['channel']):''),floor($_POST["ticket_stamp"]/2),$_POST["ticket_stamp"],$_POST["ticket_seri"],$_SESSION['id_log_current']],1);
        if($rs) {
            $_SESSION['page'] = 3;
            ...
            $.ajax({
                ...
                .done(function(msg) {
                    ...
                    if (flag == false) {
                        ...
                        // console.log(flag);
                        setTimeout(
                            "$('#btn_spin').show();flag=false;window.location.href='gift-confirm.php';$('#spinform').removeClass('spinform_bgspin');",
                            10000);
                    }
                });
